...
export default function EditRenditionModal({ bandish, index }: { bandish: any, index: number }) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setFormArtist(bandish.youtube_renditions[index]?.artist || "");
      setFormTitle(bandish.youtube_renditions[index]?.title || "");
      setFormUrl(bandish.youtube_renditions[index]?.url || "");
    } else {
            ...
            <form onSubmit={(e) => handleFormSubmit(e, 'save')} className="space-y-6 md:space-y-8">
              ...
              <div className="flex flex-col md:flex-row gap-4 items-end">
                <div className="w-full">
                  ...
                  <input type="password" required placeholder="Secret password" value={adminPasscode} onChange={(e) => setAdminPasscode(e.target.value)} className="w-full bg-m3-error/10 dark:bg-m3-error-dark/10 text-gray-900 dark:text-white px-6 py-4 rounded-[1.5rem] focus:outline-none focus:ring-2 focus:ring-m3-error dark:focus:ring-m3-error-dark transition-all duration-300 placeholder-m3-error/50 dark:placeholder-m3-error-dark/50" />
                </div>
